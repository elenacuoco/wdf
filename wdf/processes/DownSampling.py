...
class DownSampling(object):
    ...
    def __init__(self, Parameters, order=6, padlen=None, estimation=False):
        """
        The constructor
        ...
        :type padlen:int
        :padlen:  the lenght of workspace for backward filter. It must be <= the lenght of the input data
        """
        try:
        ...
        if order is None:
            self.order = 6
            logging.info("Filter order not defined, we fix it equal to 6")
        else:
            self.order = order

         # Apply a low-pass filter to the data to prevent aliasing
        self.sos = butter(self.order, self.cutoff_frequency
                          / self.nyquist_frequency, 'low', output='sos')
       ...
       # Get the steady state of the filter's step response.
        ...
        if padlen is None:
           self.padlen = int(self.sampling / 4)
           logging.info("Padlen not defined, we fix it equal to int(fs/4)")
        else:
            self.padlen = padlen

        self.prefix = np.zeros(self.padlen)
        if  estimation is False: 
            logging.info("Estimation status is set to False. If you need to estimate AR, please update the value")
