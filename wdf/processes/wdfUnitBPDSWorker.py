...
from wdf.processes.BandPassDownSampling import BanPassDownSampling
...
class wdfUnitDSWorker(object):
    ...
    def segmentProcess(self, segment, wavThresh=WaveletThreshold.dohonojohnston):
        ...
        ID = "".join([str(self.par.run),"_",str(self.par.channel),"_",str(int(gpsStart))])
        dir_chunk = "".join([self.par.outdir,ID,"/"])
